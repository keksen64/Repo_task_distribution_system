...
import tfms.utils.JSONParserToTask;
import tfms.utils.TaskUpdateStatus;
import tfms.utils.UserStatusSetter;
...
public class TaskUpdate implements HttpHandler {
    private static final Logger log = Logger.getLogger(TaskUpdate.class);
    public void handle(HttpExchange httpExchange) throws IOException {
        try {
                log.debug("start work handler");
                String rb = ExtractorRequestBody.extract(httpExchange);
                Task t1 = JSONParserToTask.parse(rb);
                log.debug("start work handler\n body: " + rb);
                String response = TaskUpdateStatus.change(t1);
